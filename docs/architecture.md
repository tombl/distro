...
`image.mkRootfs` constructs a conventional FHS tree by unioning the selected packages, files, and init program; conflicting non-identical paths are errors. Every root filesystem is an immutable squashfs image, with writable tmpfs mounts at `/run`, `/tmp`, and `/workspace`. Product artifacts are owned by their consumer: `linux-guest.image` and `runner.image` each contain their shared boot initramfs and product-specific rootfs.
