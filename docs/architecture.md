...
`image.mkRootfs` constructs a conventional FHS tree by unioning the selected packages, files, and init program; conflicting non-identical paths are errors. It produces an immutable squashfs image by default, with writable tmpfs mounts at `/run`, `/tmp`, and `/workspace`; `format = "ext4"` instead produces a persistent read-write filesystem. The default guest profile stays minimal, the site profile adds a curated interactive userland, and the standalone runner profile carries the full ported software set. Product artifacts are owned by their consumer: `linux-guest.image`, `site.image`, and `runner.image` each contain their shared boot initramfs and product-specific rootfs.
