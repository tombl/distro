...
Early kernel and libc tests boot a raw test-specific initramfs whose minimal init performs one assertion and emits a single pass marker. A small Node runner starts the machine and fails on timeout, exception, kernel panic, explicit failure, or missing pass marker. This layer deliberately stays below apk and the guest agent. Port, package-manager, and product tests boot APK-installed systems so they exercise solver metadata, ownership, installation, and the same package database visible at runtime; the package-manager test then installs from an embedded repository inside the running guest.
