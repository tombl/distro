...
export function Struct<T extends object>(layout: { [K in keyof T]: Type<T[K]> }) {
  let size = 0;
  ...
  return class {
    ...
    static {
      for (const [key, type] of Object.entries(layout as Record<PropertyKey, Type<unknown>>)) {
        const offset = size;
...
export function FixedArray<T>(type: Type<T>, length: number): Type<T[]> {
