...
stdenv.mkDerivation (finalAttrs: {
  ...
  # platform supports.
  #
  # The patch compiles out os.exec's fork()+exec child (wasm has no fork(), so it
  # would not even link) and replaces it with a stub that throws a catchable
  # error, following the tar port's loud-unsupported-path philosophy.
  patches = [ ./no-fork-exec.patch ];
