...
# Prove Git finds its installed helpers without GIT_EXEC_PATH. git-remote-http
# is a standalone binary (not a builtin symlink); https/ftp/ftps link to it.
[ "$(git --exec-path)" = /usr/libexec/git-core ] ||
  fail "unexpected exec path: $(git --exec-path)"
[ -e /usr/libexec/git-core/git-remote-http ] || fail "git-remote-http not installed"
[ -e /usr/libexec/git-core/git-remote-https ] || fail "git-remote-https not installed"
