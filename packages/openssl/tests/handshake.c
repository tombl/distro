 ...
 * libcrypto with no socket, eventfd or AF_UNIX in sight (the guest kernel has
 * none of those). A server SSL and a client SSL are wired to the two ends of a
 * BIO pair; we pump SSL_do_handshake on both sides, shuttling the ciphertext
 * each produces to the other, until both report the handshake is complete, then
 * send one application record each way and check it arrives intact.
 *
 * The server credential is an ephemeral P-256 key with a self-signed
 * certificate, built entirely in memory. The client does not verify it (there
 * is no trust store to verify against); the point of the test is that the
 * record layer, key exchange and symmetric crypto all work on wasm32. On any
 * failure the program prints an OpenSSL error and exits nonzero; on success it
 * prints "handshake ok". */
