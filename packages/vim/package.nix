...
stdenv.mkDerivation (finalAttrs: {
  ...
  # Install only the vim binary and xxd, not the multi-megabyte runtime tree.
  # The aggregate `install` target runs installrtbase, which regenerates the
  # help tags by *executing* the freshly built vim -- impossible for a wasm
  # binary the build host cannot run -- and the runtime files are not needed for
  # the terminal-editing and ex-mode use this port targets. The granular
  # installvimbin/installtools targets that skip all that live in src/Makefile,
  # not the top-level one, so descend into src first. STRIP=true stops vim's
  # Makefile from running a host `strip` on the wasm binary; nix's fixupPhase
  # strips it with the wrapped tool.
  preInstall = "cd src";
  installTargets = [
    "installvimbin"
    "installtools"
