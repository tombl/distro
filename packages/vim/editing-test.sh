...
# Exercise normal startup without -u NONE: this sources defaults.vim and proves
# that the compiled runtime path names the package's FHS location in the guest.
# shellcheck disable=SC2016 # $VIMRUNTIME is Vim syntax, not a shell expansion.
vim -es \
  -c 'call writefile([$VIMRUNTIME], "/tmp/vimruntime")' \
  -c 'qall!' </dev/null >/tmp/startup.out 2>&1 ||
  fail "normal startup failed: $(cat /tmp/startup.out)"
[ "$(cat /tmp/vimruntime)" = /usr/share/vim/vim91 ] ||
  fail "unexpected VIMRUNTIME: $(cat /tmp/vimruntime)"
[ -f /usr/share/vim/vim91/defaults.vim ] ||
  fail "vim fragment is missing defaults.vim"
