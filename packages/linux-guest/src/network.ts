...
function parse_ip(frame: Uint8Array): ParsedIpPacket | undefined {
  ...
  if ((header.flags_and_fragment_offset & 0x3fff) !== 0) return;
  return {
    source: header.source,
    destination: header.destination,
    protocol: header.protocol,
    payload: bytes.subarray(header_length, header.total_length),
  };
}
...
export function createNetwork({ connectTcp, resolveDns }: NetworkOptions): Network {
  ...
  function ip_packet(
    ...
    allocated.value = {
      ...
      identification: ip_identifier++ & 0xffff,
  ...
  async function send_tcp(flow: TcpFlow, flags: number, payload = new Uint8Array()) {
    ...
    await gateway.send(
      ip_packet(flow.guest.macAddress, flow.peer_ip, flow.guest.ip, IpProtocol.TCP, segment),
    );
  }
  ...
  async function send_tcp_data(flow: TcpFlow, data: Uint8Array) {
    for (let offset = 0; offset < data.byteLength; offset += TCP_MSS) {
      if (flow.closed || flow.local_ended) throw new Error("TCP connection is closed");
      const payload = data.slice(offset, Math.min(offset + TCP_MSS, data.byteLength));
  ...
  async function start_outbound_tcp(
    ...
    try {
      ...
      await gateway.send(
        ip_packet(guest.macAddress, ip.destination, guest.ip, IpProtocol.TCP, rst),
      );
      close_tcp_flow(flow, error);
    }
  }
  ...
  async function process_tcp(flow: TcpFlow, segment: ParsedTcpSegment) {
    ...
    if (segment.flags & TcpFlag.ACK) acknowledge_waiters(flow, segment.acknowledgement);
    ...
    await send_tcp(flow, TcpFlag.ACK);
    if (flow.local_ended && flow.remote_ended) close_tcp_flow(flow);
  }
  ...
  async function receive_dns(guest: Attachment, datagram: ParsedUdpDatagram) {
    try {
      ...
      await gateway.send(ip_packet(guest.macAddress, GATEWAY_IP, guest.ip, IpProtocol.UDP, udp));
    } catch {
  ...
  function receive_udp(ip: ParsedIpPacket) {
    ...
    udp_flows
      .get(udp_key(ip.source, datagram.source_port, datagram.destination_port))
      ?.controller.enqueue(datagram.payload.slice());
  }
  ...
  function connect_tcp(options: TcpConnectOptions): Promise<TcpConnection> {
    ...
    const timeout = setTimeout(() => {
      abort_tcp_flow(
        flow,
        new Error(`timed out connecting to ${options.hostname}:${options.port}`),
      );
    }, TCP_TIMEOUT_MS);
    void handshake.promise.finally(() => clearTimeout(timeout)).catch(() => {});
    const syn = tcp_segment(GATEWAY_IP, guest.ip, port, options.port, initial, 0, TcpFlag.SYN);
    void gateway
      .send(ip_packet(guest.macAddress, GATEWAY_IP, guest.ip, IpProtocol.TCP, syn))
      .catch((error) => {
        close_tcp_flow(flow, error);
      });
  ...
  function connect_udp(options: UdpConnectOptions): Promise<UdpConnection> {
    ...
    const writable = new WritableStream<Uint8Array>({
      async write(payload) {
        ...
        await gateway.send(
          ip_packet(guest.macAddress, GATEWAY_IP, guest.ip, IpProtocol.UDP, datagram),
        );
      },
      close,
      abort: close,
