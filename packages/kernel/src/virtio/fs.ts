...
function timestamp(value: FSTimestamp | undefined): [bigint, number] {
...
function create_context(header: FuseInHeaderValue, mode: number, umask: number): FSCreateContext {
  return {
    mode: mode & ~umask,
    uid: header.uid,
    gid: header.gid,
...
async function async_iterable<T>(source: Iterable<T> | AsyncIterable<T>): Promise<T[]> {
  const result: T[] = [];
...
export function fileSystemDevice<TNode extends object, THandle, T extends FS<TNode, THandle>>(
  ...
  function handle_record(fh: bigint, directory?: boolean, node?: NodeRecord<TNode>) {
  ...
  function add_handle(node: NodeRecord<TNode>, handle: THandle, directory: boolean) {
  ...
  async function process(header: FuseInHeaderValue, body: Reader, capacity: number) {
    ...
    switch (header.opcode) {
      ...
      case FuseOpcode.GETATTR: {
        ...
        const handle =
          request.getattr_flags & FuseGetattrFlags.FH
      ...
      case FuseOpcode.SETATTR: {
        ...
        const open =
          request.valid & FuseSetattrFlags.FH
            ? handle_record(request.fh, undefined, node!).handle
            : undefined;
      ...
      case FuseOpcode.RENAME: {
        ...
        const moved =
          node === new_parent ? undefined : await filesystem.lookup(node!.node, old_name);
