 ...
 */
export function vsockDevice({ guestCid = 3n }: { guestCid?: bigint } = {}): VsockDevice {
  ...
  function allocate_port() {
    do {
      local_port_last = ((local_port_last + 1) & ~(1 << 31)) | EPHEMERAL_PORT_BASE;
  ...
  function reset_no_sock(header: VsockHeader) {
    ...
    send_packet(header.dst_port, header.src_port, VsockOp.RST, 0, new Uint8Array(), 0, header.type);
  }

  function connection_ops(local_port: number, peer_port: number): VsockConnectionOps {
  ...
  function handle_tx_packet(header: VsockHeader, payload: Uint8Array) {
    ...
    switch (header.op) {
      ...
      case VsockOp.SHUTDOWN:
        state.peer_shutdown |= header.flags & (VsockShutdown.RCV | VsockShutdown.SEND);
  ...
  function connect(
    ...
    const connection = create_vsock_connection(connection_ops(local_port, port), local_port, port);
  ...
  function listen(
    ...
    let active = 0;
    let listening = true;
    const { promise: finished, resolve: resolve_finished } = Promise.withResolvers<void>();
