 ...
 * @param {string} options.hub hub origin, e.g. "https://hub.localhost.low.land"
...
export function serveGuest({ hub, fetch }) {
  ...
  // Everything after the first host label, e.g. ".localhost.low.land", shared by
 ...
 *
 * @param {object} options
 * @param {string} options.origin guest origin, e.g. "https://8080.localhost.low.land"
...
function connectBridge({ origin, fetch }) {
  ...
  function onMessage(event) {
    ...
    serve(channel.port1);
    iframe.contentWindow.postMessage({ type: "bridge:port" }, origin, [channel.port2]);
  }
