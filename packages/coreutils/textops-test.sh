...
contains "$(hostname --version)" "GNU coreutils" ||
  fail "hostname is not GNU coreutils"
[ -n "$(arch)" ] || fail "arch produced no machine name"
[ -n "$(hostname)" ] || fail "hostname produced no host name"
...
contains "$(env COREUTILS_TEST=ok env)" "COREUTILS_TEST=ok" ||
...
contains "$(cat /tmp/date-out)" 1970 ||
  fail "date -d @0 did not resolve to the epoch"
