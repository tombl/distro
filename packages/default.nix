...
lib.makeScope (scope: lib.callPackageWith ({ inherit lib pkgs; } // scope)) (
  ...
  in
  {
    ...
    stdenvReleaseBase = import ./stdenv.nix {
      inherit pkgs;
      inherit (self) platform llvm-toolchain sysroot;
    };
    stdenvDebugBase = pkgs.stdenvAdapters.keepDebugInfo self.stdenvReleaseBase;
    stdenvRelease = self.apk.wrapStdenv self.stdenvReleaseBase;
    stdenvDebug = self.apk.wrapStdenv self.stdenvDebugBase;
