...
for wrapper in bzgrep bzegrep bzfgrep bzdiff bzcmp bzmore bzless; do
  if [ -e "/bin/$wrapper" ]; then
    fail "unsupported shell wrapper $wrapper was shipped"
  fi
done
...
bzcat /tmp/data.bz2 >/tmp/data.bzcat || fail "bzcat alias failed"
cmp /tmp/data /tmp/data.bzcat || fail "bzcat output did not match the original"
