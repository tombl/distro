...
# 3. Parallel build of two independent targets with -j2. Both must appear.
...
# 4. $(shell ...) expansion worked: the 'world' prerequisites came from a
#    $(shell echo w1 w2), so both files existing proves $(shell) ran.

# 5. A failing recipe (false) stops the build with nonzero exit, and the
